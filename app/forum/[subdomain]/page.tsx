...
async function getForumData(subdomain: string) {
  try {
    ...
    if (!forums || forums.length === 0) {
      console.log("No forum found for subdomain:", subdomain)
      throw new Error("Not found")
    }

    if (forums.length > 1) {
      console.warn("Multiple forums found for subdomain:", subdomain, "using first one")
    }
...
async function checkForumFollow(forumId: string, userId: string | undefined): Promise<boolean> {
  if (!userId) return false
...
async function checkForumOwnership(forumId: string, userId: string | undefined): Promise<boolean> {
  if (!userId) return false
...
export default async function ForumPage({ params }: ForumPageProps) {
  ...
  try {
    ...
    const isFollowing = await checkForumFollow(forum.id, currentUser?.id)

    // Check if current user owns this forum
    const isOwner = await checkForumOwnership(forum.id, currentUser?.id)
          ...
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-8">
              <nav className="flex items-center space-x-2 text-sm">
                ...
                <Link href="/explore" className="text-cyan-400 hover:text-cyan-300 transition-colors font-medium flex items-center gap-1">
              ...
              <Card className="relative bg-black/80 border-purple-500/30 backdrop-blur-sm">
                <CardContent className="p-8">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <FollowForumButton forumId={forum.id} isFollowing={isFollowing} />
